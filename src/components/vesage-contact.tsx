...
export function ContactForm() {
 ...
 const submit=async(event:FormEvent<HTMLFormElement>)=>{
  ...
  try{const result=await submitInquiry({data:{name:String(data.get('name')??''),email:String(data.get('email')??''),brand:String(data.get('brand')??''),brief:String(data.get('brief')??''),website:String(data.get('website')??'')}});setReceipt(result.receipt);form.reset();}
 ...
 return <form onSubmit={submit} className="inquiry-form" aria-label="Start a project" aria-busy={pending}>
  ...
  <label>Brand or company <span>(optional)</span><input name="brand" autoComplete="organization" maxLength={200}/></label>
